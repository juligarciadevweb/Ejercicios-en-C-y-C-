...
{
    int res = 0, n, i; 
    int mayus = 0, minus = 0, digit = 0; 
    int numAnt = -2, numAct, numSeg = 0;
    bool antFueNum;
    char prevChar = '\0'; // Carácter previo inicializado a nulo
    n = strlen(contrasena);
    
    if(n < 6) 
	{
        res = 1;
        printf("\nERROR: La contrasenia debe tener minimo 6 caracteres, por favor ingrese nuevamente por favor.");
    }
    
    if(n > 32) 
	{
        res = 1;
        printf("\nERROR: La Contrasenia debe tener maximo 32 caracteres,Ingrese nuevamente por favor.");
    }
    ...
    for(i = 0 ; i < n ; i++) 
	{
        ...
        if(isdigit(contrasena[i])) 
		{
            ...
            if(antFueNum) 
			{
                if(numAct == numAnt+1) 
				{
                    numSeg++; //Aumenta el contador en los numeros seguidos
                    numAnt = numAct;
                    if(numSeg > 3) 
					{
                        res = 1;
                        printf("\nERROR:La Contrasena debe tener maximo 3 numeros seguidos, Ingrese nuevamente por favor.");
                    }
                } else {
                    numSeg = 0;
                    numAnt = -2;
        ...
        if(isalpha(contrasena[i]) && prevChar && (abs(contrasena[i] - prevChar) == 1)) 
		{
            res = 1;
            printf("\nERROR: No debe tener 2 caracteres consecutivos que refieran a letras alfabéticamente consecutivas, Ingrese nuevamente por favor.");
        }
		        
        prevChar = contrasena[i];
    }      
    return res;
}
